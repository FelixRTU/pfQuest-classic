...
local function DrawLine(path,x,y,nx,ny,hl,minimap)
  ...
  if minimap then
    -- player coords
    xplayer, yplayer = C_Map.GetPlayerMapPosition(C_Map.GetBestMapForUnit("player"), "player"):GetXY()
  ...
  for i=(minimap and 1 or 2), dots-(minimap and 1 or 2) do
    ...
    if minimap then
      ...
      xpos = xpos / 100 * WorldMapFrame.ScrollContainer.Child:GetWidth()
      ypos = ypos / 100 * WorldMapFrame.ScrollContainer.Child:GetHeight()
    end
    ...
    if display then
      ...
      path[nline] = path[nline] or (minimap and pfMap.drawlayer or WorldMapFrame.ScrollContainer.routes):CreateTexture(nil, "OVERLAY")
      ...
      if minimap then -- draw minimap
        ...
        path[nline]:SetPoint("CENTER", WorldMapFrame.ScrollContainer.Child, "TOPLEFT", xpos, -ypos)
      end
...
if WorldMapFrame:GetFrameStrata() == "FULLSCREEN" then
  mapStrata = "FULLSCREEN_DIALOG"
elseif WorldMapFrame:GetFrameStrata() == "MEDIUM" then
  mapStrata = "HIGH"
else
  mapStrata = "FULLSCREEN"
end
...
pfQuest.route.arrow:SetScript("OnDragStart", function(this)
  if IsShiftKeyDown() then
    this:StartMoving()
  end
end)

pfQuest.route.arrow:SetScript("OnDragStop", function(this)
