...
compatnamefake:SetScript("OnEvent", function(this)
...
pfMap.tooltip:SetScript("OnShow", function(this)
  ...
  local zone = pfMap:GetMapIDByName(C_Map.GetMapInfo(WorldMapFrame:GetMapID()).name)
...
function pfMap:SetMapByID(name)

  local setZoneName = C_Map.GetAreaInfo(name)
  local allZones = C_Map.GetMapChildrenInfo(947, 3, true) -- 947:Cosmic
  
  for _,zone in pairs(allZones) do 
    if zone.name == setZoneName then
...
local function NodeEnter(this)
  local tooltip = GameTooltip--this:GetParent() or WorldMapFrame.ScrollContainer or GameTooltip
...
function pfMap:BuildNode(name, parent)
  ...
  if parent == WorldMapFrame.ScrollContainer.Child then
  ...
  f:SetScript("OnEnter", NodeEnter)
  f:SetScript("OnLeave", NodeLeave)
...
function pfMap:UpdateNode(frame, node, color, obj)
  ...
  if frame.updateLayer then
    frame:SetFrameLevel((obj == "minimap" and 1 or 2021) + frame.layer)
  end

  if frame.updateTexture or frame.updateVertex or frame.updateColor or frame.updateLayer then
    frame:SetScript("OnClick", (frame.func or NodeClick))
  end
...
function pfMap:UpdateNodes()
  ...
  local map = pfMap:GetMapIDByName(C_Map.GetMapInfo(WorldMapFrame:GetMapID()).name)
  local i = 1
    ...
    if pfMap.nodes[addon][map] then
      for coords, node in pairs(pfMap.nodes[addon][map]) do
        if not pfMap.pins[i] then
          pfMap.pins[i] = pfMap:BuildNode("pfMapPin" .. i, WorldMapFrame.ScrollContainer.Child)
        end
        ...
        if pfMap.pins[i].cluster and pfQuest_config.showcluster == "0" then
          ...
          x = x / 100 * WorldMapFrame.ScrollContainer.Child:GetWidth()
          y = y / 100 * WorldMapFrame.ScrollContainer.Child:GetHeight()

          pfMap.pins[i]:ClearAllPoints()
          pfMap.pins[i]:SetPoint("CENTER", WorldMapFrame.ScrollContainer.Child, "TOPLEFT", x, -y)
...
local coord_cache = {}
function pfMap:UpdateMinimap(this)
  ...
  local position = C_Map.GetPlayerMapPosition(C_Map.GetMapInfo(WorldMapFrame:GetMapID()).mapID, "player")
  local xPlayer, yPlayer

  if position then
    xPlayer, yPlayer = position:GetXY()
  else
    xPlayer = 0
    yPlayer = 0
  end
    ...
    if data[mapID] and minimap_sizes[mapID] then
      for coords, node in pairs(data[mapID]) do
        ...
        if pfQuestCompat.rotateMinimap then
          local sinFacing = sin(GetPlayerFacing())
          local cosFacing = cos(GetPlayerFacing())
...
pfMap:RegisterEvent("ADDON_LOADED")
pfMap:SetScript("OnEvent", function(this, event, arg1)
  -- save current zone

  -- set map to current zone when possible
  if event == "ZONE_CHANGED" or event == "MINIMAP_ZONE_CHANGED" or event == "ZONE_CHANGED_NEW_AREA"  or event == "ZONE_CHANGED_INDOORS" then
    -- fix to load current map the first time
  ...
  if event == "ADDON_LOADED" and string.find(arg1, "pfQuest") then
    WorldMapFrame:Show()
    WorldMapFrame:Hide()
  end
end)
...
pfMap:SetScript("OnUpdate", function(this)
