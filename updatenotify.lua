...
versioncheck:SetScript("OnEvent", function(this, arg1)
  if event == "ADDON_LOADED" then
    if string.find(arg1.path, "pfQuest") then
  ...
  if version and version > 0 and pfQuest_config.latest and pfQuest_config.latest > version and not displayed then
    DEFAULT_CHAT_FRAME:AddMessage(pfQuest_Loc["|cff33ffccpf|rQuest: New version available!"])
    displayed = true
  end
end)
