...
local function ShowTooltip(this)
  if not this or not this.tooltips then return end
...
local function ResultButtonEnter(this)
  this.tex:SetColorTexture(1,1,1,.1)

  -- quest
  if this.btype == "quests" then
    pfDatabase:ShowExtendedTooltip(this, this.id, GameTooltip, this, "ANCHOR_LEFT", -10, -5)
...
local function ResultButtonClick(this, arg1)
...
local function ResultButtonLeave(this)
  ...
  if compat.mod(this:GetID(),2) == 1 then
    this.tex:SetColorTexture(1,1,1,.02)
  else
    this.tex:SetColorTexture(1,1,1,.04)
  end
  GameTooltip:Hide()
end
...
local function ResultButtonCreate(i, resultType)
  local f = CreateFrame("Button", nil, pfBrowser.tabs[resultType].list,  BackdropTemplateMixin and "BackdropTemplate")
  ...
  -- favourite button
  f.fav = CreateFrame("Button", nil, f,  BackdropTemplateMixin and "BackdropTemplate")
  ...
  if resultType == "items" then
    ...
    for button, settings in pairs(buttons) do
      f[button] = CreateFrame("Button", nil, f,  BackdropTemplateMixin and "BackdropTemplate")
...
local function SelectView(view)
  --#Done crude fix to color button text button.a = button name, button.b = search results
  for id, frame in pairs(pfBrowser.tabs) do
    frame.button:SetText("|cccffffff" ..  frame.button.a .. "|r |cffaaaaaa(" ..  frame.button.b .. ")")
    frame:Hide()
  end
  
  view.button:SetText("|cff33ffcc" .. view.button.a .. "|r |cffaaaaaa(" ..  view.button.b .. ")")
  view.button:Hide()
  view.button:Show()
  view:Show()
end
...
local function RefreshView(i, key, caption)
  ...
  --#Done crude textcolor fix
  pfBrowser.tabs[key].button.a = pfQuest_Loc[caption]
  pfBrowser.tabs[key].button.b = (i >= search_limit and "*" or i)
...
local function CreateBrowseWindow(fname, name, parent, anchor, x, y)
  ...
  parent.tabs[fname].button:SetText(name)
  
  --#Done idk (font for browser tab buttons)
  local NewFont = CreateFont(name .. "Font")
  local path, height, flags = GameFontNormal:GetFont()
  NewFont:SetFont(path, height, flags)
  NewFont:SetTextColor(1, 1, 1, 0.8)
  parent.tabs[fname].button:SetNormalFontObject(NewFont)
  
  parent.tabs[fname].button.a = name
  parent.tabs[fname].button.b = "0"
...
-- browser window
pfBrowser = CreateFrame("Frame", "pfQuestBrowser", UIParent,  BackdropTemplateMixin and "BackdropTemplate")
pfBrowser:Hide()
...
pfBrowser:SetScript("OnMouseUp",function(this)
  this:StopMovingOrSizing()
end)

pfBrowser:SetScript("OnUpdate", function(this)
...
pfBrowser.close = CreateFrame("Button", "pfQuestBrowserClose", pfBrowser,  BackdropTemplateMixin and "BackdropTemplate")
...
pfBrowser.close:SetScript("OnClick", function(this)
  this:GetParent():Hide()
end)
...
pfBrowser.journal = CreateFrame("Button", "pfQuestJournalOpen", pfBrowser,  BackdropTemplateMixin and "BackdropTemplate")
...
pfBrowser.clean = CreateFrame("Button", "pfQuestBrowserClean", pfBrowser,  BackdropTemplateMixin and "BackdropTemplate")
...
pfBrowser.input = CreateFrame("EditBox", "pfQuestBrowserSearch", pfBrowser,  BackdropTemplateMixin and "BackdropTemplate")
...
pfBrowser.input.clearButton = CreateFrame("Button", "$parentClearButton", pfBrowser.input,  BackdropTemplateMixin and "BackdropTemplate")
...
pfBrowser.input.clearButton:SetScript("OnEnter", function(this)
  this.texture:SetAlpha(1.0)
end)
pfBrowser.input.clearButton:SetScript("OnLeave", function(this)
  this.texture:SetAlpha(0.5)
end)
pfBrowser.input.clearButton:SetScript("OnMouseDown", function(this)
  if this:IsEnabled() then
    this.texture:SetPoint("TOPLEFT", this, "TOPLEFT", 1, -1)
  end
end)
pfBrowser.input.clearButton:SetScript("OnMouseUp", function(this)
...
pfBrowser.input:SetScript("OnEscapePressed", function(this) this:ClearFocus() end)
pfBrowser.input:SetScript("OnEnterPressed", function(this) this:ClearFocus() end)
pfBrowser.input:SetScript("OnEditFocusGained", function(this)
  this:HighlightText()
...
pfBrowser.input:SetScript("OnEditFocusLost", function(this)
...
pfBrowser.input:SetScript("OnTextChanged", function(this)
