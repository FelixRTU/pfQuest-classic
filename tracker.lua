...
local function ShowTooltip(this)
  if this.tooltip then
    ...
    if this.node then
      this.node.questid = this.node.questid or this.questid
      local tmpqid = string.find(this.questid,"^[0-9]*$") and this.questid or this.node.realid

      if pfDB["quests"] and pfDB["quests"]["loc"] and pfDB["quests"]["loc"][tmpqid] and pfDB["quests"]["loc"][tmpqid]["O"] then
        GameTooltip:AddLine(pfDatabase:FormatQuestText(pfDB["quests"]["loc"][tmpqid]["O"]), 1,1,1,1)
        GameTooltip:AddLine(" ")
      end
...
tracker:SetScript("OnEvent", function(this)
...
tracker:SetScript("OnMouseDown",function(this) this:StartMoving() end)
...
function tracker.ButtonUpdate(this)
  ...
  if not this.alpha or this.alpha ~= alpha then
    this.bg:SetColorTexture(0,0,0,alpha)
  ...
  if pfMap.highlight and pfMap.highlight == this.title then
    if not this.highlight then
      this.bg:SetColorTexture(1,1,1,math.max(.2, alpha))
...
function tracker.ButtonClick(this, arg1)
  ...
  elseif IsControlKeyDown() and not WorldMapFrame:IsShown() then
    -- show world map
    local maps = pfDatabase:SearchQuestID(this.questid, nil)
...
function tracker.ButtonEvent(self)
  ...
  if tracker.mode == "QUEST_TRACKING" then
    ...
    local qtitle, level, tag, header, collapsed, complete = GetQuestLogTitle(qlogid)
    ...
    if objectives and unfolded[title] then --or ( percent > 0 and percent < 100 )) then
...
function tracker.ButtonAdd(title, node)
  ...
  elseif tracker.mode == "GIVER_TRACKING" then -- skip everything that isn't a questgiver
    ...
    if node.layer and node.layer > 2 then return end
  ...
  --#Done: strange fix to show objective tooltips in quest tracker
  tracker.buttons[id].node.questid = node.questid
  tracker.buttons[id].node.realid = node.questid
