...
pfQuest:SetScript("OnEvent", function(this, event, arg1)
  if event == "ADDON_LOADED" then
    if string.find(arg1, "pfQuest") then
      pfQuest:AddQuestLogIntegration()
      pfQuest:AddWorldMapIntegration()
      pfQuestMiniMapButton.Init();
...
pfQuest:SetScript("OnUpdate", function(this)
...
function pfQuest:ResetAll()
  ...
  pfQuest.updateQuestGivers = true
  pfMap:UpdateNodes()
  pfQuestMiniMapButton.UpdateVisibility();
end
...
function pfQuest:AddQuestLogIntegration()
  ...
  pfQuest.buttonOnline:SetScript("OnClick", function(this)

    local questurl = "https://classic.wowhead.com/quest=" .. (this:GetID() or 0)
    if client > 11306 then
      questurl = "https://tbc-twinhead.twinstar.cz/?quest=" .. (this:GetID() or 0)
    end
    
    DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffff" .. pfQuest_Loc["Online quest search"] .. ": |cff00ff00|Hurl:" .. questurl .. "|h" .. questurl .. "|h")
  end)
  ...
  --pfQuest.buttonLanguage:SetScript("OnClick", function(this)
  ...
  --pfQuest.buttonLanguage:SetScript("OnUpdate", function(this)
...
function pfQuest:AddWorldMapIntegration()
  ...
  pfQuest.mapButton = CreateFrame("Frame", "pfQuestMapDropdown", WorldMapFrame.ScrollContainer, "UIDropDownMenuTemplate")
  pfQuest.mapButton:ClearAllPoints()
  --#ToDo somethings wrong with that anchors
  pfQuest.mapButton:SetPoint("TOPRIGHT",  WorldMapFrame.ScrollContainer, "TOPRIGHT", -117, -30)
  --pfQuest.mapButton:SetPoint("TOPLEFT" , -5, -10)
  ...
  pfQuest.mapButton.point = "TOPRIGHT"
  pfQuest.mapButton.relativePoint = "TOPRIGHT"
  ...
  if pfQuest_config["questloglevel"] == "1" then
    for i=1, QUESTS_DISPLAYED, 1 do
      ...
      if display <= entries then
        local title, level, tag, header = GetQuestLogTitle(display)
        if not header then
...
if not GetQuestLink then -- Allow to send questlinks from questlog
  ...
  QuestLogTitleButton_OnClick = function(this, button)
    ...
    if IsShiftKeyDown() and not this.isHeader and ChatFrame1EditBox:IsVisible() then
    ...
    pfHookQuestLogTitleButton_OnClick(this, button)
  end
